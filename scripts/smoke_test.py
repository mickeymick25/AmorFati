...
if not index.exists():
    ...
    if 'rel="manifest"' not in text or "manifest.json" not in text:
        errors.append(
            "manifest link not found or not pointing to manifest.json in index.html"
        )
    # Service worker registration is now in app.js
    sw_text = (
        (root / "app.js").read_text(encoding="utf-8")
        if (root / "app.js").exists()
        else ""
    )
    if "serviceWorker" not in text and "serviceWorker" not in sw_text:
        errors.append(
            "service worker registration code not found in index.html or app.js"
        )
    if "service-worker.js" not in text and "service-worker.js" not in sw_text:
        errors.append("service-worker.js reference not found in index.html or app.js")
    # check nav-tab buttons (class may span multiple lines)
    nav_count = text.count("nav-tab")
    if nav_count < 4:
